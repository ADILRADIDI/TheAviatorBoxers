...
test("header shows À propos as second link and footer shows managed contacts", async ({ page }, testInfo) => {
  ...
  await expect(footer.getByText("contact@theaviatorboxer.com")).toBeVisible();
...
test("admin can manage site settings and the storefront reflects them", async ({ page }, testInfo) => {
  ...
  await expect(emailInput).toHaveValue("contact@theaviatorboxer.com");
  ...
  await page.evaluate(async () => {
    const response = await fetch("/api/admin/settings", {
      ...
      body: JSON.stringify({ email: "contact@theaviatorboxer.com" }),
    });
