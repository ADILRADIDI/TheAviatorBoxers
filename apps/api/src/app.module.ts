...
import { adminSessions, adminUserRoles, adminUsers, auditLogs, categories, cmsPages, colors, coupons, createDatabase, inventoryMovements, mediaAssets, notifications, orders, permissions, productVariants, products, promotions, returnRequests, reviews, rolePermissions, roles, shippingZones, siteSettings } from "@aviator/db";
...
class AppController {
  ...
  async product(@Param("slug") slug: string) {
    ...
    if (!row) return null;
    const flat = { ...row, price: row.price / 100, status: "active", color_name: row.colorName, sizes: row.sizes || ["M", "L", "XL", "XXL"], featured: row.featured, images: row.images || [], category: "boxer", category_id: row.categoryId, description: row.description || "", review_count: reviewCount, rating: reviewRating != null ? Number(reviewRating) : null };
    const variants = await db.select().from(productVariants).where(and(eq(productVariants.productId, row.id), eq(productVariants.active, true))).orderBy(productVariants.size);
    if (variants.length === 0) return flat;
    const names = [...new Set(variants.map((v) => v.color).filter(Boolean))];
    const colorRows = names.length ? await db.select().from(colors).where(and(eq(colors.active, true), ...names.map((n) => eq(colors.name, n)))) : [];
    const byName = new Map(colorRows.map((c) => [c.name, c]));
    const colorsOut = names.map((n, i) => byName.get(n) ?? { id: null, name: n, hex: null, active: true, sortOrder: 0 }).sort((a, b) => (a.sortOrder ?? 0) - (b.sortOrder ?? 0));
    return {
      ...flat,
      colors: colorsOut,
      variants: variants.map((v) => ({ id: v.id, size: v.size, color: v.color, sku: v.sku, stock: v.stock, active: v.active, price: v.price ? v.price / 100 : flat.price })),
    };
  }

  @Get("api/colors")
  async colorsList() {
    return db.select().from(colors).where(eq(colors.active, true)).orderBy(colors.sortOrder);
  }
  ...
  async sitemap(@Res() response: any) {
    ...
    const staticPaths = ["/", "/notre-boxer", "/pourquoi-nous", "/avis", "/contact", "/livraison-retours", "/guide-des-tailles", "/paiement", "/faq", "/cgv", "/confidentialite"];
  ...
  @Post("api/reviews")
  async createReview(@Body() body: { name: string; city?: string; rating: number; comment: string; product_id?: string; tissu?: number; service?: number; livraison?: number; qualite?: number }) {
    const crit = (v: any) => (v == null ? null : Math.max(1, Math.min(5, Number(v) || 5)));
    const [row] = await db.insert(reviews).values({ name: body.name, city: body.city, rating: body.rating, comment: body.comment, tissu: crit(body.tissu), service: crit(body.service), livraison: crit(body.livraison), qualite: crit(body.qualite), productId: body.product_id, status: "pending", verified: false }).returning();
  ...
  async deleteProduct(@Param("id") id: string) {
    const variants = await db.select({ id: productVariants.id }).from(productVariants).where(eq(productVariants.productId, id));
    if (variants.length) {
      const vIds = variants.map((v) => v.id);
      await db.delete(inventoryMovements).where(sql`${inventoryMovements.variantId} IN (${sql.join(vIds.map((id) => sql`${id}`), sql`, `)})`);
  ...
  async moderateReview(@Param("id") id: string, @Body() body: { status: string }) {
    const [row] = await db.update(reviews).set({ status: body.status, verified: body.status === "approved" }).where(eq(reviews.id, id)).returning();
    await audit("review.moderated", "review", id, { status: body.status });
    return row;
  }

  @Post("api/admin/reviews")
  async createReviewAdmin(@Body() body: { name: string; city?: string; rating: number; comment: string; product_id?: string; status?: string; tissu?: number; service?: number; livraison?: number; qualite?: number }) {
    const crit = (v: any) => (v == null ? null : Math.max(1, Math.min(5, Number(v) || 5)));
    const status = body.status || "approved";
    const [row] = await db.insert(reviews).values({ name: body.name, city: body.city, rating: Math.max(1, Math.min(5, Number(body.rating) || 5)), comment: body.comment, tissu: crit(body.tissu), service: crit(body.service), livraison: crit(body.livraison), qualite: crit(body.qualite), productId: body.product_id, status, verified: status === "approved" }).returning();
