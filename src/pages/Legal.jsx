...
const CGV = [
  ...
  { title: "Article 2 — Produits", text: "Les produits proposés sont des sous-vêtements pour hommes, confectionnés en 95% coton et 5% Élasthanne. Les caractéristiques, prix et disponibilités sont indiqués sur chaque fiche produit." },
  ...
  return (
    <>
      <PageHeader
        eyebrow={eyebrow}
        title={title}
        subtitle={`${t("Dernière mise à jour :")} ${new Date().toLocaleDateString("fr-FR")}`}
        image={isCGV ? "/images/story-workshop.jpg" : "/images/diff-label.jpg"}
      />
        ...
        <div className="mx-auto max-w-3xl space-y-8">
          {sections.map((s, i) => (
            <section key={i} className="group">
              <h2 className="flex items-center gap-2.5 font-heading text-lg">
                <span className="h-2 w-2 shrink-0 bg-[#C7D400] transition-colors group-hover:bg-[#C7D400]" />
                {t(s.title)}
              </h2>
