...
export default function About() {
  ...
  const { t } = useLanguage();
  usePageMeta({ title: "À propos — The Aviator", description: "L'histoire de The Aviator : des boxers premium pour hommes, inspirés par l'exigence de l'aéronautique. 95% coton, 5% Élasthanne. Fabriqué avec soin." });
      ...
      <section className="py-16 lg:py-24">
        <div className="container-edge grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <div>
            ...
            <h2 className="mt-2 font-heading text-3xl font-bold tracking-tight sm:text-4xl">{t("La précision au service du confort")}</h2>
      ...
      <section className="bg-secondary py-16 lg:py-24">
        <div className="container-edge">
          <Reveal className="text-center">
            ...
            <h2 className="mt-2 font-heading text-3xl font-bold tracking-tight sm:text-4xl">{t("Ce qui nous guide")}</h2>
          </Reveal>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {VALUES.map((v, i) => (
              <Reveal key={v.title} delay={i * 0.08}>
                <div className="group h-full border border-border bg-background p-8 text-center">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center bg-foreground/[0.04] transition-colors duration-300 group-hover:bg-[#C7D400]">
        ...
        <div className="container-edge grid grid-cols-2 gap-8 text-center lg:grid-cols-4">
          {[
            ...
            <Reveal key={i} delay={i * 0.08}>
              <p className="font-heading text-4xl text-[#C7D400] lg:text-5xl">{s.n}</p>
