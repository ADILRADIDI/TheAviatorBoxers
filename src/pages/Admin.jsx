...
import {
  Activity, Bell, ChevronLeft, Compass, Images, Layers, LayoutDashboard, Lock, Megaphone,
  Menu, Package, Palette, ScrollText, Settings, ShieldCheck, ShoppingBag, Star, Tags, TicketPercent,
  Trash2, Truck, UserCog, Users, Warehouse, X,
} from "lucide-react";
import { AdminColorsPanel, AdminProductsPanel } from "@/components/admin/AdminCatalogPanels";
...
import AdminPixelsPanel from "@/components/admin/AdminPixelsPanel";
...
const navGroups = [
  { title: "Pilotage", items: [["dashboard", "Vue d'ensemble", LayoutDashboard], ["acquisition", "Sources de trafic", Compass], ["commandes", "Commandes", ShoppingBag], ["customers", "Clients", Users], ["audit-logs", "Journal d'activité", ScrollText], ["notifications", "Notifications", Bell], ["roles", "Rôles & permissions", ShieldCheck], ["users", "Utilisateurs admin", UserCog]] },
  { title: "Catalogue", items: [["produits", "Produits", Package], ["colors", "Couleurs", Palette], ["variants", "Variantes & stock", Layers], ["inventory", "Inventaire", Warehouse]] },
  { title: "Vente", items: [["shipping-zones", "Livraison", Truck], ["coupons", "Coupons", TicketPercent]] },
  { title: "Contenu", items: [["avis", "Avis clients", Star]] }, 
  { title: "Configuration", items: [["settings", "Réglages", Settings], ["pixels", "Pixels & Tracking", Activity]] },
];
const tabs = navGroups.flatMap((group) => group.items);
const apiTabs = { commandes: "orders", acquisition: "orders", produits: "products", "shipping-zones": "shipping-zones", coupons: "coupons", avis: "reviews", inventory: "inventory", notifications: "notifications", "audit-logs": "audit-logs", settings: "settings", pixels: "settings" };
const navPermission = { dashboard: "dashboard.view", acquisition: "dashboard.view", commandes: "orders.view", customers: "customers.view", "audit-logs": "audit_logs.view", notifications: "notifications.view", roles: "roles.view", users: "users.view", produits: "products.view", colors: "products.view", variants: "variants.view", inventory: "inventory.view", "shipping-zones": "shipping.view", coupons: "discounts.view", avis: "reviews.view", settings: "settings.view", pixels: "settings.view" };
const tabToRoute = { dashboard: "dashboard", acquisition: "traffic", commandes: "orders", customers: "clients", "audit-logs": "activity", notifications: "notifications", roles: "roles", users: "users", produits: "products", colors: "colors", variants: "variants", inventory: "inventory", "shipping-zones": "shipping", coupons: "coupons", avis: "reviews", settings: "settings", pixels: "pixels" };
const routeToTab = Object.fromEntries(Object.entries(tabToRoute).map(([tab, route]) => [route, tab]));
routeToTab.returns = "commandes";
routeToTab.acquisition = "acquisition";
routeToTab.traffic = "acquisition";
          ...
          <div className="flex items-center justify-between gap-3 px-5 py-3.5 sm:px-8">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-0.5 truncate font-heading text-xl font-extrabold tracking-tight text-navy">{activeLabel}</h1>
              </div>
            </div>
            <div className="flex items-center gap-2.5">
              ...
              <div className="hidden items-center gap-2.5 rounded-md border border-black/10 py-1 pl-1 pr-3 sm:flex">
                <span className="flex h-7 w-7 items-center justify-center rounded bg-navy font-heading text-xs font-black text-accent-lime">A</span>
            ...
            <AdminErrorBoundary>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {tab === "acquisition" && <AdminTrafficPanel />}
                  ...
                  {tab === "colors" && <AdminColorsPanel data={Array.isArray(data) ? data : []} refresh={load} />}
                  ...
                  {tab === "pixels" && <AdminPixelsPanel />}
                  {tab === "images" && <AdminStorefrontImagesPanel />}
                  {tab === "cms" && <AdminCmsPanel data={Array.isArray(data) ? data : []} refresh={load} />}
  ...
  return (
    <>
      <div className="flex h-16 items-center gap-3 border-b border-white/10 px-4">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/10 p-1.5 ring-1 ring-white/15">
          <img src="/logo.png" alt="The Aviator" className="h-full w-auto object-contain brightness-200" />
        </div>
        {!collapsed && (
          <div className="min-w-0">
            <p className="truncate font-heading text-sm font-black uppercase tracking-[0.18em]">The Aviator</p>
...
function AdminLogin({ onSuccess }) {
  const [form, setForm] = useState({ email: "admin@theaviator.local", password: "Aviator-Admin2026!" });
  const [error, setError] = useState("");
    ...
    <div data-admin-shell className="flex min-h-screen bg-[hsl(0_0%_96%)]">
      <div className="relative hidden w-[42%] flex-col justify-between overflow-hidden bg-[hsl(0_0%_5%)] p-10 text-white lg:flex">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10 p-2 ring-1 ring-white/20">
            <img src="/logo.png" alt="The Aviator" className="h-full w-auto object-contain brightness-200" />
          </div>
          <div>
            <p className="font-heading text-sm font-black uppercase tracking-[0.2em]">The Aviator</p>
        ...
        <div className="relative z-10">
          ...
          <h2 className="mt-3 font-heading text-3xl font-black leading-tight tracking-tight">L'outil de gestion de la<br />boutique haut de gamme.</h2>
          ...
          <div className="mt-8 flex flex-wrap gap-8">
            <div>
              <strong className="font-heading text-2xl font-black text-accent-lime">100%</strong>
              <p className="mt-0.5 text-[11px] uppercase tracking-wider text-white/45">COD · Paiement à la livraison</p>
            </div>
            <div>
              <strong className="font-heading text-2xl font-black text-accent-lime">24h</strong>
        ...
        <form onSubmit={submit} className="admin-pop w-full max-w-md">
          <div className="lg:hidden">
            <div className="mb-6 flex items-center gap-3">
              ...
              <div><p className="font-heading text-xs font-black uppercase tracking-[0.2em]">The Aviator</p><p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Espace administrateur</p></div>
            </div>
          </div>
...
function Reviews({ data, refresh }) {
  const items = Array.isArray(data) ? data : [];
  const [form, setForm] = useState({ name: "", city: "", rating: "5", comment: "", product_id: "", tissu: "5", service: "5", livraison: "5", qualite: "5" });
  ...
  const createReview = async (event) => {
    ...
    setForm({ name: "", city: "", rating: "5", comment: "", product_id: form.product_id, tissu: "5", service: "5", livraison: "5", qualite: "5" });
    refresh();
  };
  ...
  return (
    <Panel title="Avis clients" eyebrow="Contenu généré">
      ...
      <form onSubmit={createReview} className="mt-4 grid gap-3 sm:grid-cols-[1.4fr_1fr_2fr_1fr_3fr_auto]">
        ...
        <div className="grid gap-3 sm:col-span-6 sm:grid-cols-4">
          {[{ key: "tissu", label: "Tissu" }, { key: "service", label: "Service" }, { key: "livraison", label: "Livraison" }, { key: "qualite", label: "Qualité" }].map((c) => (
            <select key={c.key} value={form[c.key]} onChange={(e) => setForm({ ...form, [c.key]: e.target.value })} className="admin-input" aria-label={c.label}>
              {[5, 4, 3, 2, 1].map((r) => <option key={r} value={r}>{c.label} — {r}/5</option>)}
            </select>
          ))}
        </div>
