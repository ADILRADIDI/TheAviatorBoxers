...
export default function ProductCard({ product, index = 0 }) {
  ...
  const firstColor = product.colors?.[0]?.name || product.color_name || "Bleu marine";
  const cardVariants = product.variants?.length ? product.variants.filter((v) => v.color === firstColor) : [];
  const cardSizes = cardVariants.length ? [...new Set(cardVariants.map((v) => v.size))] : product.sizes;
  const cardSize = cardSizes?.[1] || cardSizes?.[0] || "M";
  const cardVariant = cardVariants.find((v) => v.size === cardSize);
  const cardStock = cardVariant ? cardVariant.stock : (product.stock || 0);

  const quickAdd = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (soldOut || (cardVariants.length > 0 && !cardVariant)) return;
    addItem({
      productId: product.id,
      slug: product.slug,
      name: product.name,
      image: mainImage,
      price: cardVariant ? cardVariant.price : product.price,
      color: firstColor,
      size: cardSize,
      quantity: 1,
      stock: cardStock,
      ...
      <div className="group block">
        <Link to={`/produit/${product.slug}`} className="block">
          <div className="relative aspect-[3/4] overflow-hidden bg-muted">
            ...
            {/* Badges */}
            {!soldOut && pct > 0 ? (
              <span className="absolute left-3 top-3 bg-[#C7D400] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-navy">
                -{pct}%
              </span>
            ) : null}
            {product.badge && (
