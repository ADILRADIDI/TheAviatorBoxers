...
// Floating WhatsApp affordance with a soft pulse ring.
export default function WhatsAppFloat() {
  const [visible, setVisible] = useState(false);
  const [whatsappUrl, setWhatsappUrl] = useState(() => whatsappContactUrl());
  const { t } = useLanguage();
  ...
  return (
    <a
      ...
      className={`group fixed bottom-5 right-5 z-30 flex items-center justify-center rounded-full bg-[#25D366] text-white shadow-[0_16px_40px_-12px_rgba(0,0,0,0.55)] transition-all duration-300 hover:-translate-y-0.5 ${visible ? "translate-y-0 opacity-100" : "translate-y-16 opacity-0"}`}
