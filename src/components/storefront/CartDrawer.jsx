          ...
          >
            ...
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold uppercase tracking-[0.15em]">Panier</h2>
                {itemCount > 0 && <span className="text-xs text-muted-foreground">({itemCount})</span>}
              </div>
              <button onClick={closeDrawer} className="p-1.5 text-navy" aria-label="Fermer le panier">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            {items.length === 0 ? (
              <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">Découvrez notre collection premium.</p>
                </div>
                <Link
                  to="/collection"
                  onClick={closeDrawer}
              ...
              <>
                ...
                {freeShipDiff > 0 ? (
                  ...
                  <div className="border-b px-5 py-3">
                    <p className="text-xs font-semibold text-accent-lime">✓ Livraison gratuite débloquée</p>
                  </div>
                )}
                        ...
                        <li key={key} className="flex gap-3">
                          ...
                          <div className="flex flex-1 flex-col">
                            <div className="flex justify-between gap-2">
                              ...
                              <button onClick={() => removeItem(key)} className="text-muted-foreground hover:text-destructive" aria-label="Retirer">
                                <X className="h-4 w-4" />
                              </button>
                            </div>
                            <div className="mt-auto flex items-center justify-between pt-2">
                              <div className="flex items-center border">
                                <button onClick={() => updateQuantity(key, item.quantity - 1)} className="flex h-7 w-7 items-center justify-center text-navy hover:bg-muted" aria-label="Diminuer">
                                  <Minus className="h-3 w-3" />
                                </button>
                                <span className="w-8 text-center text-sm font-medium">{item.quantity}</span>
                                <button onClick={() => updateQuantity(key, item.quantity + 1)} className="flex h-7 w-7 items-center justify-center text-navy hover:bg-muted" aria-label="Augmenter">
                ...
                <div className="border-t px-5 py-4">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Livraison calculée à l'étape suivante</p>
                  <Link
                    to="/checkout"
                    onClick={closeDrawer}
