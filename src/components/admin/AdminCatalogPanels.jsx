...
const emptyProduct = { name: "", slug: "", price: "", stock: "", color_name: "Bleu marine", images: [], description: "", featured: false };
...
export function AdminProductsPanel({ data, refresh }) {
  ...
  useEffect(() => {
    adminRequest("/api/admin/colors")
      .then((result) => setColors(Array.isArray(result) ? result : result?.data || []))
      .catch(() => {}); // ponytail: couleurs optionnelles, le texte libre reste valide
  }, []);
  ...
  const colorOptions = [...new Set(colors.map((color) => color.name).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  const hasCurrentColor = form.color_name && !colorOptions.includes(form.color_name);
