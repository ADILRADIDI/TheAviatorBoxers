      ...
      <div className="container-edge grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
        ...
        >
          ...
          <div className="absolute -bottom-4 -right-4 hidden bg-accent-lime px-6 py-4 text-navy sm:block">
            <p className="text-2xl font-bold">{t("2 pièces")}</p>
            <p className="text-xs font-medium uppercase tracking-wider">{t("au choix")}</p>
          </div>
        </motion.div>

        <div className="order-1 lg:order-2">
          <span className="label-eyebrow text-white/50">{t("Le pack signature")}</span>
          <h2 className="mt-2 font-display text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
            {t("Composez votre pack")}
          </h2>
          <p className="mt-5 max-w-md text-white/70">
            {t("Choisissez vos couleurs, vos tailles, et profitez d'un tarif avantageux.")}
            {t("Deux pièces The Aviator, à votre image.")}
          </p>
          <ul className="mt-6 space-y-3">
            {["2 couleurs au choix", "Tailles personnalisables", "Prix pack avantageux"].map((label) => (
          ...
          >
            {t("Composer mon pack")} <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
